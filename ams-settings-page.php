...
// Define the setting args
  ...
  // Create our settings field for reCAPTCHA Site Key
  add_settings_field( 
    'ams_plugin_site_key', 
    'Site Key',
    'ams_plugin_setting_site_key', 
    'ams_general_settings_options', 
    'ams_plugin_main' 
  );
    // Create our settings field for reCAPTCHA Secret Key
  add_settings_field( 
    'ams_plugin_secret_key', 
    'Secret Key',
    'ams_plugin_setting_secret_key', 
    'ams_general_settings_options', 
    'ams_plugin_main' 
  );
}
...
function ams_plugin_setting_fav_holiday() {
  ...
  $options = get_option('ams_general_settings_options', [ 'fav_holiday' => 'Halloween' ] );
...
// Get option 'beast_mode' value from the database
  ...
  $options = get_option( 'ams_general_settings_options', [ 'beast_mode' => 'disabled' ] );
...
// Define the radio button options
  ...
  // Loop the two radio button options and select if set in the option value
   ...
   // Display and fill the Site Key
function ams_plugin_setting_site_key() {
  ...
  // Echo the field
  echo "<input id='site_key' name='ams_plugin_options[site_key]'
      type='text' size='60' value='" . esc_attr( $site_key ) . "' />";

}

   // Display and fill the secret Key
function ams_plugin_setting_secret_key() {
  ...
  // Echo the field
  echo "<input id='secret_key' name='ams_plugin_options[secret_key]'
      type='text' size='60' value='" . esc_attr( $secret_key ) . "' />";
...
function ams_plugin_validate_options( $input ) {
  $input = $_POST['ams_plugin_options'];
// Only allow letters and spaces for the name
  ...
  $valid['fav_holiday'] = wp_strip_all_tags(sanitize_text_field( $input['fav_holiday'] ));
  $valid['beast_mode'] = wp_strip_all_tags(sanitize_text_field( $input['beast_mode'] ));
  $valid['site_key'] = wp_strip_all_tags(sanitize_text_field( $input['site_key'] ));
  $valid['secret_key'] = wp_strip_all_tags(sanitize_text_field( $input['secret_key'] ));
  $valid['pickdate'] = wp_strip_all_tags(sanitize_text_field( $input['pickdate'] ));
  $valid['pickedate'] = wp_strip_all_tags(sanitize_text_field( $input['pickedate'] ));
  return $valid;
}
?>